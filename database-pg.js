// database-pg.js - Optimized with Indexes and Better Queries
...
class DeviceDatabase {
  ...
  async initTables() {
    try {
      ...
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_devices_code ON devices(code)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_devices_status ON devices(status)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_devices_last_ping ON devices(last_ping DESC)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_codes_status ON codes(status)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_codes_is_active ON codes(is_active)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_code_hwids_code_hwid ON code_hwids(code, hwid)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_hwid_logs_created_at_desc ON hwid_logs(created_at DESC)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_devices_hwid_code ON devices(hwid, code)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_new_hwid_registry_status_detected ON new_hwid_registry(status, detected_at DESC)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_devices_status_code ON devices(status, code)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_codes_username_access ON codes(username, access_level)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_devices_created_at ON devices(created_at DESC)`);
      await this.query(`CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_codes_created_at ON codes(created_at DESC)`);
  ...
  getCachedData() {
    return {
      ...
      stats: this.cache.stats || { 
        total: 0, approved: 0, revoked: 0, totalPings: 0, 
        totalCodes: 0, activeCodes: 0, pendingRequests: 0,
        svip_count: 0, vip_count: 0
      },
  ...
  async registerDeviceWithCode(deviceId, userAgent, ip, browserInfo, code, hwid = null, hardware = null, wallpaper = null) {
    try {
      ...
      if (hwid) {
        const existingHwid = await this.query(
      ...
      if (existingDevice) {
        await this.query(
          `UPDATE devices SET 
            ...
            browser_profile = $6, cpu_name = $7, gpu_name = $8,
            ram_total_gb = $9, storage_total_gb = $10, profile_name = $11,
            device_name = $12, wallpaper_name = $13, wallpaper_size_kb = $14,
            wallpaper_width = $15, wallpaper_height = $16, wallpaper_base64 = $17,
            approved_at = CURRENT_TIMESTAMP, revoked_at = NULL
          WHERE device_id = $18`,
          [userAgent || '', ip || '', JSON.stringify(browserInfo || {}), code,
           hwid, profileName, cpuName, gpuName, ramTotal, storageTotal,
           profileName, deviceName, wallpaperName, wallpaperSizeKb,
           wallpaperWidth, wallpaperHeight, wallpaperBase64, deviceId]
        );
      } else {
        await this.query(
          `INSERT INTO devices (
            device_id, user_agent, ip_address, browser_info, code, hwid,
            status, approved_at, browser_profile, cpu_name, gpu_name,
            ram_total_gb, storage_total_gb, profile_name, device_name,
          ...
          [deviceId, userAgent || '', ip || '', JSON.stringify(browserInfo || {}),
           code, hwid, profileName, cpuName, gpuName, ramTotal, storageTotal,
           profileName, deviceName, wallpaperName, wallpaperSizeKb,
           wallpaperWidth, wallpaperHeight, wallpaperBase64]
        );
      }

      await this.markHwidAsAssigned(hwid, code);
      await this.query('UPDATE codes SET used_count = used_count + 1 WHERE code = $1', [code]);
      await this.logUsage(deviceId, code, 'register', `Device registered | Profile: ${profileName}`);
      await this.refreshCache();
  ...
  async assignHwidToCode(code, hwid, autoAssign = false) {
    try {
      ...
      const otherCode = await this.query(
        'SELECT code FROM code_hwids WHERE hwid = $1 AND code != $2',
        [hwid, code]
      );
      if (otherCode.rows[0]) {
        return { success: false, error: `HWID is already assigned to code: ${otherCode.rows[0].code}` };
      }
      ...
      if (currentCount >= limit) {
        if (!autoAssign) {
          return { 
            success: false, 
            error: `HWID limit reached (${limit})`,
            limit_reached: true,
  ...
  async autoDeactivateCode(code, reason = 'unauthorized_use') {
    try {
      ...
      const devices = await this.query('SELECT device_id FROM devices WHERE code = $1', [code]);
      ...
      return { success: true, code, devices_revoked: devices.rowCount, reason, status };
    } catch (error) {
  ...
  async deactivateCode(code) {
    try {
      const devices = await this.query('SELECT device_id FROM devices WHERE code = $1 AND status != $2', [code, 'revoked']);
      await this.query('DELETE FROM code_hwids WHERE code = $1', [code]);
      ...
      await this.refreshCache();
      return { success: result.rowCount > 0, devicesRemoved: devices.rowCount };
    } catch (error) {
  ...
  async reactivateCode(code, subscriptionType = 'Lifetime') {
    try {
      ...
      const result = await this.query(
        `UPDATE codes SET is_active = true, status = 'active', subscription_type = $1,
         subscription_started_at = $2, expires_at = $3 WHERE code = $4`,
  ...
  async updateCodeSubscription(code, subscriptionType) {
    try {
      ...
      const result = await this.query(
        `UPDATE codes SET subscription_type = $1, subscription_started_at = $2, expires_at = $3,
         status = 'active', is_active = true WHERE code = $4`,
        [subscriptionType, now, expiresAt, code]
      );
  ...
  async logHwidActivity(hwid, code, deviceId, action, status, details, ip, userAgent, browserProfile) {
    try {
      ...
      if (assigned.rows[0] && assigned.rows[0].code) {
        return true;
      }
  ...
  async getHwidLogs(limit = 200) {
    try {
      const result = await this.query(
        'SELECT * FROM hwid_logs ORDER BY created_at DESC LIMIT $1',
        [limit]
      );
      return result.rows;
    } catch (error) {
