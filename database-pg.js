...
class DeviceDatabase {
  ...
  async registerDeviceWithCode(deviceId, userAgent, ip, browserInfo, code, hwid = null, hardware = null) {
    try {
      ...
      return { 
        ...
        allow_wink: !!(updatedCodeInfo.allow_wink === true || updatedCodeInfo.allow_wink === 't' || updatedCodeInfo.allow_wink === 1),
  ...
  async updateCodeAllowWink(code, allow) {
    try {
      const result = await this.run(
        'UPDATE codes SET allow_wink = $1 WHERE code = $2',
        [!!allow, code]
      );
      await this.refreshCache();
      return !!(result && result.changes > 0);
    } catch (error) {
      console.error('Update allow_wink error:', error);
      return false;
    }
  }
